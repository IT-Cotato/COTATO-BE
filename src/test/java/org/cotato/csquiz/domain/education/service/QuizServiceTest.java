...
class QuizServiceTest {
	...
	void findAllQuizzes_withMultipleAndShortQuizzes_returnsMappedResponses() {
		...
		List<Choice> dummyChoices = List.of(Choice.of(1, "1", ChoiceCorrect.ANSWER, multipleQuiz),
			Choice.of(2, "2", ChoiceCorrect.ANSWER, multipleQuiz));
		when(choiceReader.getChoicesByMultipleQuizzes(List.of(multipleQuiz))).thenReturn(
			Map.of(multipleQuiz, dummyChoices));

		List<ShortAnswer> dummyAnswers = List.of(ShortAnswer.of("answer1", shortQuiz),
			ShortAnswer.of("answer2", shortQuiz));
		when(shortAnswerReader.getAnswersByShortQuizzes(List.of(shortQuiz))).thenReturn(
			Map.of(shortQuiz, dummyAnswers));
